    ...
    def do_POST(self):
        try:
            ...
            if not re.match(r'https://docs\.google\.com/spreadsheets/d/[a-zA-Z0-9-_]+', sheets_url):
                self.send_error_response(400, {'success': False, 'error': 'URL inválida. Use formato: https://docs.google.com/spreadsheets/d/...'})
                return
            
            # Extrai ID da planilha
            sheet_id_match = re.search(r'/spreadsheets/d/([a-zA-Z0-9-_]+)', sheets_url)
            if not sheet_id_match:
                self.send_error_response(400, {'success': False, 'error': 'ID da planilha não encontrado na URL'})
                return
            ...
            if response.status_code == 200 and len(response.text) > 50:
                ...
                self.send_error_response(400, {'success': False, 'error': 'Planilha não está pública ou não existe'})
                
        except requests.exceptions.Timeout:
            self.send_error_response(408, {'success': False, 'error': 'Timeout após 30s - planilha demorou muito para responder'})
